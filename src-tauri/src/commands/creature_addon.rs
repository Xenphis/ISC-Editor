...
pub async fn save_creature_addon(
    ...
    const SQL: &str = "REPLACE INTO creature_addon (guid, path_id, mount, MountCreatureID, StandState, AnimTier, VisFlags, SheathState, PvPFlags, emote, visibilityDistanceType, auras) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    debug_sql!(app, debug, SQL,
        sqlx::query(SQL)
        .bind(guid)
        .bind(addon.path_id)
